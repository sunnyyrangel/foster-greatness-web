...
const bodySchema = z.object({
  ...
  connection_rating: z.number().int().min(1).max(4).optional(),
  program_id: z.string().min(1),
  program_name: z.string().optional(),
  source: z.enum(['findhelp', 'community']).optional(),
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { error } = await supabase.from('resource_feedback').insert(validation.data);

    if (error && process.env.NODE_ENV === 'development') {
      console.error('Resource feedback insert error:', error);
    }
