-- Migration: Search function for informational_resources
-- Provides text search with relevance ranking and geography filtering.

-- ============================================================================
-- zip_to_state: USPS ZIP prefix → 2-letter state code
-- ============================================================================

CREATE OR REPLACE FUNCTION zip_to_state(zip text)
RETURNS text
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE
    -- Extract 3-digit prefix for range matching
    WHEN zip IS NULL OR length(trim(zip)) < 3 THEN NULL
    ELSE (
      SELECT state FROM (VALUES
        -- Source: USPS ZIP prefix ranges
        ...
        ('525','WI'), ('526','WI'), ('527','WI'), ('528','WI'),
        ('530','WI'), ('531','WI'), ('532','WI'), ('534','WI'), ('535','WI'),
        ('537','WI'), ('538','WI'), ('539','WI'),
        ('540','MN'), ('541','MN'), ('543','MN'), ('544','MN'), ('545','MN'),
        ('546','MN'), ('547','MN'), ('548','MN'), ('549','MN'),
        ...
        ('585','MT'), ('586','MT'), ('587','MT'), ('588','MT'), ('589','MT'),
        ...
        ('640','KS'), ('641','KS'), ('644','KS'), ('645','KS'), ('646','KS'),
        ('647','KS'), ('648','KS'), ('649','KS'),
        ('650','MO'), ('651','MO'), ('652','MO'), ('653','MO'), ('654','MO'),
        ('655','MO'), ('656','MO'), ('657','MO'), ('658','MO'), ('659','MO'),
        ...
        ('885','TX'), ('889','NV'), ('890','NV'), ('891','NV'), ('893','NV'),
      ...
      WHERE prefix = left(trim(zip), 3)
    )
  END;
$$;

COMMENT ON FUNCTION zip_to_state(text) IS 'Converts a ZIP code to a 2-letter US state code using USPS 3-digit prefix ranges.';

-- ============================================================================
-- search_informational_resources: Full-text + ILIKE search with geography
-- ============================================================================

CREATE OR REPLACE FUNCTION search_informational_resources(
...
AS $$
  SELECT
    ...
    CASE
      ...
      ELSE (
        ...
        CASE WHEN r.title ILIKE '%' || search_text || '%' THEN 3.0 ELSE 0.0 END
        -- Tag match
        + CASE WHEN EXISTS (
            SELECT 1 FROM unnest(r.tags) AS t
            WHERE t ILIKE '%' || search_text || '%'
          ) THEN 2.0 ELSE 0.0 END
        -- Description match
        + CASE WHEN r.description ILIKE '%' || search_text || '%' THEN 1.0 ELSE 0.0 END
        -- Full-text ranking boost
        + COALESCE(
            ts_rank(
              to_tsvector('english', coalesce(r.title, '') || ' ' || coalesce(r.description, '') || ' ' || array_to_string(r.tags, ' ')),
  ...
  FROM informational_resources r
  WHERE r.is_active = true
    -- Geography filter: NULL state returns all, otherwise national + state-specific
    AND (user_state IS NULL OR r.geography = 'national' OR r.geography = user_state)
    -- Text filter: NULL search returns all, otherwise require at least one match
    AND (
      search_text IS NULL
      ...
      OR to_tsvector('english', coalesce(r.title, '') || ' ' || coalesce(r.description, '') || ' ' || array_to_string(r.tags, ' '))
         @@ plainto_tsquery('english', search_text)
    )
  ORDER BY relevance DESC, r.title ASC;
$$;

COMMENT ON FUNCTION search_informational_resources(text, text) IS 'Search informational resources by text with geography filtering. Uses ILIKE for partial matches and tsvector for relevance ranking.';
